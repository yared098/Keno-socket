...
function generateProfitableNumbers(userSelections, totalDraws = 20, numberRange = 80) {
  ...
  const selectionPool = numberStats.slice(0, numberRange); // pool from least picked numbers
  const maxAttempts = 1000;
  const maxAllowedWinners = Math.max(1, Math.floor(userSelections.length * 0.1));
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (potentialWinners.length <= maxAllowedWinners) {
      winners = potentialWinners.sort((a, b) => b.matches.length - a.matches.length);
      break;
    }
  }
